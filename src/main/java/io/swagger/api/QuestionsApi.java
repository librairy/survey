...
public interface QuestionsApi {
    ...
    @ApiOperation(value = "", notes = "Create a new `Question` ", response = Boolean.class, tags={  })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "questions of a survey", response = Boolean.class)})
    @RequestMapping(value = "/questions",
            produces = { "application/json" },
            method = RequestMethod.POST)
    ResponseEntity<Boolean> questionsPost(@RequestBody Question question);

    @ApiOperation(value = "", notes = "Remove all `Question`s of a `Survey`. ", response = Boolean.class, tags={  })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "questions of a survey", response = Boolean.class)})
    @RequestMapping(value = "/questions",
            produces = { "application/json" },
            method = RequestMethod.DELETE)
    ResponseEntity<Boolean> questionsDelete();
