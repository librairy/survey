...
public interface AnswersApi {
    ...
    @ApiOperation(value = "", notes = "Read a list of  `Answer`s ", response = AnswerList.class, tags={  })
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "answer list", response = AnswerList.class) })
    @RequestMapping(value = "/answers",
            produces = { "application/json" },
            method = RequestMethod.GET)
    ResponseEntity<AnswerList> answersGet(
            @ApiParam(value = "number of answers" ,required=true ) @RequestParam Integer size,
            @ApiParam(value = "offset" ,required=false ) @RequestParam String page
            );
