...
public class QuestionsApiController implements QuestionsApi {
    ...
    @Override
    public ResponseEntity<Boolean> questionsPost(@RequestBody Question question) {
        return new ResponseEntity(service.saveQuestion(question), HttpStatus.OK);
    }

    @Override
    public ResponseEntity<Boolean> questionsDelete() {
        return new ResponseEntity(service.deleteQuestions(), HttpStatus.OK);
    }
}
