...
public class AnswersApiController implements AnswersApi {
    ...
    @Override
    public ResponseEntity<AnswerList> answersGet(@ApiParam(value = "number of answers", required = true) @RequestParam Integer size, @ApiParam(value = "offset", required = false) @RequestParam String page) {
        return new ResponseEntity<AnswerList>(service.getAnswers(size,page),HttpStatus.OK);
    }
